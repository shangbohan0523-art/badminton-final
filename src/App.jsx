...
const PRO_PLAYERS = [
  { name: '林丹', gender: 'M', style: '全能控制', attrs: { attr_power: 9, attr_speed: 9, attr_endurance: 9, attr_mentality: 10, attr_forehand: 9, attr_backhand: 8, attr_attack: 10, attr_defense: 9 }, honors: ['双圈全满贯得主'], description: 'GOAT。早期暴力突击，后期控制大师。', quote: '只有坚持到最后的人，才能看到对手倒下。' },
  { name: '李宗伟', gender: 'M', style: '极致速度', attrs: { attr_power: 7, attr_speed: 10, attr_endurance: 9, attr_mentality: 7, attr_forehand: 9, attr_backhand: 8, attr_attack: 10, attr_defense: 8 }, honors: ['三届奥运银牌'], description: '拥有羽坛最顶级的步法和弹跳。', quote: '败者为王，永不言弃。' },
  { name: '陶菲克', gender: 'M', style: '反手天才', attrs: { attr_power: 7, attr_speed: 7, attr_endurance: 6, attr_mentality: 8, attr_forehand: 8, attr_backhand: 10, attr_attack: 8, attr_defense: 7 }, honors: ['奥运会男单冠军'], description: '羽坛反手天花板，网前手感细腻。', quote: '天赋是上天给的，但风格是自己创造的。' },
  { name: '安赛龙', gender: 'M', style: '强力进攻', attrs: { attr_power: 10, attr_speed: 7, attr_endurance: 9, attr_mentality: 9, attr_forehand: 9, attr_backhand: 7, attr_attack: 10, attr_defense: 8 }, honors: ['两届奥运冠军'], description: '拥有绝对的身高优势和极具威胁的下压进攻。', quote: '每一次杀球都是对胜利的渴望。' },
  { name: '傅海峰', gender: 'M', style: '重炮手', attrs: { attr_power: 10, attr_speed: 8, attr_endurance: 8, attr_mentality: 9, attr_forehand: 9, attr_backhand: 6, attr_attack: 10, attr_defense: 7 }, honors: ['四届世锦赛冠军'], description: '左手重炮，后场进攻能力处于人类巅峰。', quote: '唯有进攻，才能掌握命运。' },
  { name: '戴资颖', gender: 'F', style: '魔术师', attrs: { attr_power: 6, attr_speed: 8, attr_endurance: 8, attr_mentality: 7, attr_forehand: 10, attr_backhand: 9, attr_attack: 8, attr_defense: 6 }, honors: ['亚运会金牌'], description: '羽坛魔术师，假动作极多，球路诡谲莫测。', quote: '快乐打球，享受比赛。' },
  { name: '安洗莹', gender: 'F', style: '天才防守', attrs: { attr_power: 7, attr_speed: 8, attr_endurance: 10, attr_mentality: 9, attr_forehand: 8, attr_backhand: 8, attr_attack: 7, attr_defense: 10 }, honors: ['世锦赛冠军'], description: '拥有令人绝望的防守能力和跑动耐力。', quote: '只要球没落地，比赛就没有结束。' },
  { name: '陈雨菲', gender: 'F', style: '大帝稳健', attrs: { attr_power: 7, attr_speed: 8, attr_endurance: 9, attr_mentality: 10, attr_forehand: 8, attr_backhand: 8, attr_attack: 7, attr_defense: 9 }, honors: ['奥运会冠军'], description: '技术全面，失误极少，打法极其稳健。', quote: '稳住心态，就能赢下一切。' },
  { name: '马林', gender: 'F', style: '狮子吼', attrs: { attr_power: 9, attr_speed: 10, attr_endurance: 8, attr_mentality: 9, attr_forehand: 9, attr_backhand: 7, attr_attack: 10, attr_defense: 7 }, honors: ['奥运会冠军'], description: '以极快的速度和凶悍的进攻著称。', quote: '在场上，我就是女王。' },
];
...
const calculatePower = (player) => {
  if (!player) return 0;
  const attrSum = ATTRS.reduce((acc, curr) => acc + (Number(player[curr.key]) || 5), 0);
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-full">
      <svg width={size} height={size} className="overflow-visible">
        {levels.map(l => (
          <polygon key={l} points={ATTRS.map((_, i) => { 
            const r = (l / 10) * radius; 
        ...
        <motion.polygon 
          ...
          animate={{ opacity: 0.6, scale: 1, points: ATTRS.map((attr, i) => { 
            const value = data[attr.key] || 5; 
            const r = (value / 10) * radius; 
            return `${center + r * Math.cos(i * angleStep - Math.PI / 2)},${center + r * Math.sin(i * angleStep - Math.PI / 2)}`; 
          }).join(' ') }} 
          transition={{ type: "spring", stiffness: 50 }} 
          fill={color} stroke={color} strokeWidth="2" fillOpacity="0.3" 
        />
        {ATTRS.map((attr, i) => { 
          const rLabel = radius + 18; 
          const x = center + rLabel * Math.cos(i * angleStep - Math.PI / 2); 
          const y = center + rLabel * Math.sin(i * angleStep - Math.PI / 2); 
          return (
            <foreignObject key={i} x={x - 25} y={y - 10} width="50" height="20">
              <div className="text-[9px] text-white/60 text-center flex justify-center items-center">{attr.label}</div>
            </foreignObject>
          ) 
        })}
      </svg>
...
const NavBtn = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 transition-all ${active ? 'text-emerald-400' : 'text-white/40'}`}>
    {icon}
...
const TabBtn = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${active ? 'bg-emerald-500 text-white shadow-lg' : 'bg-transparent text-slate-400 hover:bg-white/5 hover:text-white'}`}>
    {icon} {label}
  </button>
);
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
        ...
        <motion.div 
          ...
          className="relative w-full max-w-md bg-slate-900 border border-white/10 p-0 rounded-3xl shadow-2xl max-h-[85vh] overflow-hidden flex flex-col"
        >
          {children}
        </motion.div>
      </div>
...
const ScoreCard = ({ team, players, score, color, onPlus, onMinus, onScoreChange, checkWin }) => (
  <div className={`flex-1 relative overflow-hidden rounded-[1.5rem] border border-white/5 bg-white/5 p-3 md:p-4 shadow-xl flex flex-col transition-all`}>
    <div className={`absolute top-0 right-0 w-24 h-24 ${color} blur-[60px] opacity-30`} />
    <div className="relative z-10 flex flex-col h-full justify-between">
      <div className="flex justify-between items-start">
        <div className="flex -space-x-2">
          {players.map(p => <Avatar key={p.id} name={p.name} colorIdx={p.avatar_idx} size="sm" className="border-slate-900 ring-2 ring-slate-900" />)}
        </div>
        <span className="text-[10px] font-black opacity-30 tracking-widest text-white mt-1">TEAM {team}</span>
      </div>
      <div className="flex items-end justify-between mt-1 md:mt-2">
        <button onClick={onMinus} className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center text-white/30 active:scale-90 touch-manipulation"><Minus size={24}/></button>
        <input type="number" inputMode="numeric" pattern="\d*" value={score} onChange={(e)=>onScoreChange(e.target.value)} onBlur={checkWin} onKeyDown={(e)=>{if(e.key==='Enter')checkWin()}} className="w-20 text-6xl leading-none font-black tracking-tighter drop-shadow-2xl font-mono text-center bg-transparent outline-none appearance-none text-white z-20 p-0 m-0" />
        <button onClick={onPlus} className={`w-14 h-14 md:w-16 md:h-16 ${color} rounded-2xl flex items-center justify-center shadow-lg active:scale-90 transition-transform text-white touch-manipulation`}><Plus size={32} strokeWidth={3} /></button>
      </div>
    </div>
  </div>
);
...
export default function App() {
  ...
  const stats = useMemo(() => {
    if (!players || !matches) return { player: [], pair: [], team: [], rising: [], raw: [] };
  ...
  const autoBalanceTeams = () => {
    const p = selectedPlayers.map(id => stats.raw.find(x => x.id === id)).filter(Boolean); if (p.length < 4) return;
    const combs = [{ a: [p[0], p[1]], b: [p[2], p[3]] }, { a: [p[0], p[2]], b: [p[1], p[3]] }, { a: [p[0], p[3]], b: [p[1], p[2]] }];
    let best = combs[0]; let minDiff = Infinity; combs.forEach(c => { const diff = Math.abs((c.a[0].rating + c.a[1].rating) - (c.b[0].rating + c.b[1].rating)); if (diff < minDiff) { minDiff = diff; best = c; } });
  ...
  const handlePlayerSave = async (e) => { e.preventDefault(); const form = new FormData(e.target); const payload = {}; ['pname','pteam','gender'].forEach(k => payload[k==='pname'?'name':k==='pteam'?'team_name':k] = form.get(k)); ATTRS.forEach(a => payload[a.key] = form.get(a.key)); if (playerModal.type === 'add') await supabase.from('players').insert([{ ...payload, avatar_idx: Math.floor(Math.random() * COLORS.length) }]); else await supabase.from('players').update(payload).eq('id', playerModal.data.id); setPlayerModal({ show: false, type: 'add', data: null }); fetchData(); };
  ...
  const prediction = useMemo(() => { if(view!=='match') return null; const getPower = (ids) => ids.reduce((sum, id) => sum + calculatePower(stats.raw.find(p=>p.id===id)), 0); const pA = getPower(matchData.teamA); const pB = getPower(matchData.teamB); const total = pA + pB || 1; const handicap = calculateHandicapValue(pA, pB); return { rateA: Math.round(pA/total*100), rateB: Math.round(pB/total*100), handicap }; }, [view, matchData, stats]);
  const calendarGrid = useMemo(() => { const year = calendarDate.getFullYear(); const month = calendarDate.getMonth(); const daysInMonth = new Date(year, month + 1, 0).getDate(); const firstDay = new Date(year, month, 1).getDay(); const matchDays = new Set(); matches.forEach(m => { const d = new Date(m.created_at); if (d.getFullYear() === year && d.getMonth() === month) matchDays.add(d.getDate()); }); return { daysInMonth, firstDay, matchDays, year, month }; }, [calendarDate, matches]);

  if (!supabase) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white p-8 text-center">Supabase not configured</div>;
  ...
  return (
    <>
      ...
      <div className="h-[100dvh] w-full bg-slate-950 flex flex-col overflow-hidden text-white font-sans">
        ...
        {view !== 'match' && (<header className="relative z-20 px-5 pt-12 pb-4 flex justify-between items-center bg-slate-950/90 backdrop-blur-md border-b border-white/5 shrink-0"><h1 onClick={()=>setView('lobby')} className="text-2xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-300 cursor-pointer select-none">SMASH.</h1><button onClick={()=>setExpenseModal(true)} className="bg-white/5 p-2.5 rounded-full hover:bg-white/10 active:scale-95 transition-transform"><Wallet size={20} className="text-emerald-400"/></button></header>)}
        ...
        <main className="relative z-10 flex-1 overflow-y-auto no-scrollbar w-full">
           <div className="w-full max-w-xl mx-auto px-4 pt-4 pb-32 flex flex-col gap-4">
            {view === 'lobby' && (
              <>
                <div className="flex justify-between items-center"><div className="flex bg-slate-900/80 p-1 rounded-xl border border-white/10 backdrop-blur-sm"><button onClick={()=>setLobbyTab('player')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${lobbyTab==='player'?'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20':'text-slate-400 hover:text-white'}`}>选手</button><button onClick={()=>setLobbyTab('pair')} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${lobbyTab==='pair'?'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20':'text-slate-400 hover:text-white'}`}>组合</button></div><div className="flex gap-3"><button onClick={() => setIsManageMode(!isManageMode)} className={`p-2.5 rounded-full transition-colors ${isManageMode ? 'bg-emerald-500 text-white' : 'bg-white/5 text-white/60 border border-white/5'}`}><Settings size={20} /></button><button onClick={() => setPlayerModal({ show: true, type: 'add' })} className="bg-white/5 border border-white/5 p-2.5 rounded-full text-emerald-400"><UserPlus size={20} /></button></div></div>
                ...
                {lobbyTab === 'player' ? (<div className="grid grid-cols-1 gap-3">{stats.player.map(p => { const isSel = selectedPlayers.includes(p.id); return (
                  <div key={p.id} className="relative group">
                    <div onClick={() => !isManageMode && (isSel ? setSelectedPlayers(prev => prev.filter(id=>id!==p.id)) : selectedPlayers.length < 4 && setSelectedPlayers(prev=>[...prev, p.id]))} className={`w-full p-3 pr-4 rounded-2xl border flex items-center gap-4 transition-all shadow-lg active:scale-[0.98] duration-200 ${isSel ? 'bg-emerald-500/10 border-emerald-500/50 ring-1 ring-emerald-500/30' : 'bg-slate-800/40 border-white/5'} ${isManageMode ? 'opacity-100' : ''}`}>
                      <Avatar name={p.name} colorIdx={p.avatar_idx} size="md"/>
                      <div className="text-left flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5"><span className="font-bold text-white truncate text-base">{p.name}</span><span className="text-[10px] font-black bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded">{p.rating}</span></div>
                        <div className="text-xs text-white/40 flex items-center gap-2 truncate"><span className="truncate max-w-[100px]">{p.team_name}</span>{p.similarPro && <span className="text-emerald-400/80 bg-emerald-500/10 px-1.5 rounded flex items-center gap-0.5 text-[10px] shrink-0"><Star size={9}/> {p.similarPro.name}</span>}</div>
                        <RecentForm results={p.recent} />
                      </div>
                      {!isManageMode ? (<button onClick={(e)=>{e.stopPropagation(); setRadarModal({show:true, data:p})}} className="p-2.5 bg-white/5 text-emerald-400/80 hover:bg-emerald-500 hover:text-white rounded-full z-20 transition-colors border border-white/5 shrink-0"><Info size={18}/></button>) : (
                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
                           <button onClick={(e) => {e.stopPropagation(); setPlayerModal({ show: true, type: 'edit', data: p })}} className="p-2.5 bg-blue-600 rounded-full shadow-lg text-white"><Edit2 size={16} /></button>
                           <button onClick={(e) => {e.stopPropagation(); handleDeletePlayer(p.id)}} className="p-2.5 bg-red-600 rounded-full shadow-lg text-white"><Trash2 size={16} /></button>
                        </div>
                      )}
                ...
                )})}</div>) : (<div className="grid grid-cols-1 gap-3">{stats.pair.length > 0 ? stats.pair.map(pair => (<div key={pair.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 pr-4 flex items-center gap-4 relative group active:scale-[0.98] transition-transform" onClick={()=>setRadarModal({show:true, data:pair, isPair:true})}><div className="flex -space-x-3 shrink-0"><Avatar name={pair.p1?.name} colorIdx={pair.p1?.avatar_idx} size="xs"/><Avatar name={pair.p2?.name} colorIdx={pair.p2?.avatar_idx} size="xs"/></div><div className="flex-1 min-w-0"><div className="text-sm font-bold text-white truncate">{pair.name}</div><div className="text-xs text-white/40">{pair.matches} 场</div><RecentForm results={pair.recent} /></div><div className="text-right shrink-0"><div className="text-emerald-400 font-black text-lg">{pair.rating}</div></div></div>)) : <div className="col-span-full text-center text-white/30 py-10">暂无固定组合数据</div>}</div>)}
                <AnimatePresence>{(() => { const radar = getRadarData(); return radar ? (<motion.button initial={{y:50, opacity:0}} animate={{y:0, opacity:1}} exit={{y:50, opacity:0}} onClick={()=>setRadarModal({show:true, data: radarData.data})} className="lg:hidden fixed bottom-36 right-4 bg-indigo-500 text-white p-3 rounded-full shadow-lg z-30 border border-white/10 flex items-center gap-2 text-sm font-bold pr-5"><Activity size={18} /> 战术分析</motion.button>) : null })()}</AnimatePresence>
              </>
            )}
            ...
            {view === 'match' && prediction && (
               <div className="h-full flex flex-col pb-6">
                 ...
                 <div className="mb-4 px-2 shrink-0"><div className="flex justify-between text-[10px] font-bold text-white/40 mb-1 uppercase tracking-wider"><span>AI Prediction</span><span>{prediction.rateA}% VS {prediction.rateB}%</span></div><div className="h-1.5 bg-slate-800 rounded-full overflow-hidden flex mb-2"><div style={{width: `${prediction.rateA}%`}} className="h-full bg-emerald-500 transition-all duration-1000"></div><div style={{width: `${prediction.rateB}%`}} className="h-full bg-blue-500 transition-all duration-1000"></div></div>{prediction.handicap && (<div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-2 flex items-center justify-center gap-2 text-xs text-purple-300"><Scale size={14}/> 建议让分: <strong>Team {prediction.handicap.team}</strong> 让 <strong>{prediction.handicap.points}</strong> 分</div>)}</div>
                 <div className="flex-1 flex flex-col gap-3 justify-center"><ScoreCard team="A" players={matchData.teamA.map(id=>stats.raw.find(p=>p.id===id)||{name:'?',colorIdx:0})} score={matchData.scoreA} color="bg-emerald-500" onPlus={()=>handleUpdateScore('A',1)} onMinus={()=>handleUpdateScore('A',-1)} onScoreChange={(val)=>handleDirectScoreChange('A',val)} /><div className="text-center font-black text-xl text-white/20 italic h-6 shrink-0">VS</div><ScoreCard team="B" players={matchData.teamB.map(id=>stats.raw.find(p=>p.id===id)||{name:'?',colorIdx:0})} score={matchData.scoreB} color="bg-blue-500" onPlus={()=>handleUpdateScore('B',1)} onMinus={()=>handleUpdateScore('B',-1)} onScoreChange={(val)=>handleDirectScoreChange('B',val)} /></div>
        ...
        <Modal isOpen={playerModal.show && playerModal.type === 'add'}><form onSubmit={handlePlayerSave} className="space-y-6 max-h-[80vh] overflow-y-auto no-scrollbar"><h3 className="text-lg font-bold text-center sticky top-0 bg-slate-900 pb-2 z-10">创建新选手</h3><div className="grid grid-cols-2 gap-3"><div className="col-span-2"><label className="text-xs text-white/40 block mb-1">昵称</label><input name="pname" className="w-full bg-black/30 rounded-lg p-3 border border-white/10 outline-none focus:border-emerald-400 text-base" required /></div><div className="col-span-2"><label className="text-xs text-white/40 block mb-1">队伍</label><input name="pteam" className="w-full bg-black/30 rounded-lg p-3 border border-white/10 outline-none focus:border-emerald-400 text-base" /></div><div className="col-span-2"><label className="text-xs text-white/40 block mb-1">性别</label><select name="gender" className="w-full bg-black/30 rounded-lg p-3 border border-white/10 outline-none focus:border-emerald-400 text-white text-base"><option value="M">男</option><option value="F">女</option></select></div>{ATTRS.map(attr => (<div key={attr.key} className="col-span-2 md:col-span-1"><div className="flex justify-between mb-1"><label className="text-xs text-white/60 flex items-center gap-1">{attr.icon} {attr.label}</label><span className="text-xs font-mono text-emerald-400 font-bold">0-10</span></div><input name={attr.key} type="range" min="0" max="10" defaultValue={5} className="w-full accent-emerald-500 h-6 bg-white/10 rounded-lg cursor-pointer"/></div>))}</div><div className="flex gap-3 pt-4 sticky bottom-0 bg-slate-900 pt-2"><button type="button" onClick={()=>setPlayerModal({show:false})} className="flex-1 py-3 bg-white/5 rounded-xl">取消</button><button type="submit" className="flex-1 py-3 bg-emerald-500 rounded-xl font-bold">保存</button></div></form></Modal>
        
        <Modal isOpen={playerModal.show && playerModal.type === 'edit'}>
          {playerModal.data ? (
            <div className="flex flex-col h-full">
              <div className="shrink-0 bg-slate-900 border-b border-white/10 p-6 text-center sticky top-0 z-20 shadow-2xl">
                 <Avatar name={playerModal.data.name} colorIdx={playerModal.data.avatar_idx} size="xl" className="mx-auto mb-3 shadow-xl"/>
                 <h3 className="text-2xl font-black text-white">{playerModal.data.name}</h3>
                 <p className="text-xs text-white/40 mt-1">正在编辑资料</p>
              </div>
              <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                <form id="edit-form" onSubmit={handlePlayerSave} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2"><label className="text-xs text-white/40 block mb-1">昵称</label><input name="pname" defaultValue={playerModal.data.name} className="w-full bg-black/30 rounded-xl p-3 border border-white/10 outline-none text-base focus:border-emerald-400 transition-colors" /></div>
                    <div className="col-span-2"><label className="text-xs text-white/40 block mb-1">队伍</label><input name="pteam" defaultValue={playerModal.data.team_name} className="w-full bg-black/30 rounded-xl p-3 border border-white/10 outline-none text-base focus:border-emerald-400 transition-colors" /></div>
                    <div className="col-span-2"><label className="text-xs text-white/40 block mb-1">性别</label><select name="gender" defaultValue={playerModal.data.gender} className="w-full bg-black/30 rounded-xl p-3 border border-white/10 outline-none text-white text-base focus:border-emerald-400 transition-colors"><option value="M">男</option><option value="F">女</option></select></div>
                    <div className="col-span-2 pt-2 border-t border-white/10 mt-2"><h4 className="text-sm font-bold text-emerald-400 mb-3 flex items-center gap-2"><Activity size={14}/> 属性调整 (0-10)</h4></div>
                    {ATTRS.map(attr => (<div key={attr.key} className="col-span-2 md:col-span-1"><div className="flex justify-between mb-1"><label className="text-xs text-white/60 flex items-center gap-1">{attr.icon} {attr.label}</label><span className="text-xs font-mono text-emerald-400 font-bold">0-10</span></div><input name={attr.key} type="range" min="0" max="10" defaultValue={playerModal.data[attr.key]||5} className="w-full accent-emerald-500 h-6 bg-white/10 rounded-lg cursor-pointer"/></div>))}
                  </div>
                </form>
                <div className="pt-4 border-t border-white/10">
                  <button onClick={()=>handleDeletePlayer(playerModal.data.id)} className="w-full py-3 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-colors rounded-xl font-bold flex items-center justify-center gap-2"><Trash2 size={16}/> 删除此选手</button>
                </div>
              </div>
              <div className="shrink-0 p-4 border-t border-white/10 bg-slate-900 grid grid-cols-2 gap-3">
                <button onClick={()=>setPlayerModal({show:false})} className="py-3 bg-white/10 rounded-xl font-bold text-sm">取消</button>
                <button form="edit-form" className="py-3 bg-emerald-500 rounded-xl font-bold text-sm text-white shadow-lg shadow-emerald-500/20">保存修改</button>
              </div>
            </div>
          ) : null}
        </Modal>
