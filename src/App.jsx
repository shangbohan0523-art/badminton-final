...
const PRO_PLAYERS = [
  { 
    ...
    honors: ['两届奥运会男单冠军', '五届世锦赛男单冠军', '全英赛六冠王'],
    description: '羽毛球历史上的GOAT。早期以极具爆发力的“变速突击”打法统治赛场，后期转型为“太极控制”打法。你的风格与他相似，意味着极强的统治力。',
  ...
  { 
    ...
    honors: ['三届奥运会男单银牌', '保持世界排名第一周数纪录', '47个超级赛冠军'],
    description: '以鬼魅般的步伐和极致的突击速度著称。防守反击能力极强，你是球场上的“法拉利”。',
    quote: '败者为王，永不言弃。'
  },
  { 
    name: '陶菲克', 
    ...
    honors: ['奥运会男单冠军', '世锦赛男单冠军', '亚运会男单金牌'],
    description: '羽坛反手天花板，拥有教科书般的反手杀球和细腻的网前手感。打球潇洒写意。',
  ...
  { 
    ...
    honors: ['奥运会男单冠军', '世锦赛男单冠军', '巡回赛收割机'],
    description: '拥有绝对的身高优势和极具威胁的下压进攻，同时练就了密不透风的防守。',
    quote: '每一次杀球都是对胜利的渴望。'
  },
  { 
    name: '傅海峰', 
    ...
    description: '左手重炮，后场进攻能力处于人类巅峰。你是队伍中的核武器。',
    quote: '唯有进攻，才能掌握命运。'
  },
  { 
    name: '戴资颖', 
    ...
    description: '羽坛魔术师，假动作极多，球路诡谲莫测。看你打球是一种享受。',
    quote: '快乐打球，享受比赛。'
  },
  { 
    name: '安洗莹', 
    ...
    description: '拥有令人绝望的防守能力和跑动耐力。你的韧性是对手的噩梦。',
  ...
  { 
    ...
    honors: ['奥运会女单冠军', '苏迪曼杯主力', '全运会卫冕冠军'],
    description: '技术全面，失误极少，打法极其稳健。场上的定海神针。',
    quote: '稳住心态，就能赢下一切。'
  },
  { 
    name: '马林', 
    ...
    honors: ['奥运会女单冠军', '三届世锦赛女单冠军', '欧洲女皇'],
    description: '以极快的速度和凶悍的进攻著称。你的球风彪悍，气势逼人。',
    quote: '在场上，我就是女王。'
  },
];
...
export default function App() {
  ...
  return (
    <>
      <GlobalStyles />
      {/* 外层容器：负责居中和背景 */}
      <div className="h-[100dvh] w-full bg-black flex justify-center overflow-hidden font-sans">
        
        {/* 手机壳容器：限制最大宽度 */}
        <div className="w-full max-w-md h-full bg-slate-950 relative flex flex-col shadow-2xl overflow-hidden">
          ...
          {/* 主内容区域 */}
          <main className={`relative z-10 w-full flex-1 overflow-y-auto no-scrollbar px-4 pt-4 pb-32`}>
                ...
                {lobbyTab === 'player' ? (
                    ...
                    {stats.player.map(p => {
                      ...
                      return (
                        <div key={p.id} className="relative group" onClick={() => !isManageMode && (isSel ? setSelectedPlayers(prev => prev.filter(id=>id!==p.id)) : selectedPlayers.length < 4 && setSelectedPlayers(prev=>[...prev, p.id]))}>
                          <div className={`w-full p-2.5 pr-3 rounded-2xl border flex items-center gap-3 transition-all shadow-lg active:scale-[0.98] duration-200 ${isSel ? 'bg-emerald-500/10 border-emerald-500/50 ring-1 ring-emerald-500/30' : 'bg-slate-800/40 border-white/5'} ${isManageMode ? 'opacity-60' : ''}`}>
                            ...
                            <div className="text-left flex-1 min-w-0">
                              ...
                              <div className="text-xs text-white/40 flex items-center gap-2 truncate">
                                <span className="truncate max-w-[80px]">{p.team_name}</span>
                  ...
                  <div className="grid grid-cols-1 gap-3 pb-4">
                     {stats.pair.length > 0 ? stats.pair.map(pair => (
                        <div key={pair.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3 relative group active:scale-[0.98] transition-transform" onClick={()=>setRadarModal({show:true, data:pair, isPair:true})}>
            ...
            {view === 'rank' && (
              <div className="space-y-6 pb-10">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {rankTab === 'player' && stats.player.map((p, i) => (<div key={p.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3 active:scale-[0.99] transition-transform" onClick={()=>setRadarModal({show:true, data:p})}><div className={`w-6 text-center font-black text-lg ${i<3?'text-yellow-400':'text-white/20'}`}>{i+1}</div><Avatar name={p.name} colorIdx={p.avatar_idx} size="sm" /><div className="flex-1 min-w-0"><div className="font-bold text-white truncate">{p.name}</div><RecentForm results={p.recent} /></div><div className="text-right"><div className="text-emerald-400 font-black text-lg">{p.score}</div><div className="text-[10px] text-white/30">PTS</div></div></div>))}
                  {rankTab === 'team' && stats.team.map((t, i) => (<div key={t.name} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3"><div className={`w-6 text-center font-black text-lg ${i<3?'text-yellow-400':'text-white/20'}`}>{i+1}</div><div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center font-bold text-sm">{t.name[0]}</div><div className="flex-1"><div className="font-bold text-white">{t.name}</div><div className="text-xs text-white/40">{t.matches}场</div></div><div className="text-right"><div className="text-emerald-400 font-black text-lg">{t.score}</div><div className="text-[10px] text-white/30">总分</div></div></div>))}
                  {rankTab === 'pair' && stats.pair.map((t, i) => (<div key={t.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3 active:scale-[0.99] transition-transform" onClick={()=>setRadarModal({show:true, data:t, isPair:true})}><div className={`w-6 text-center font-black text-lg ${i<3?'text-yellow-400':'text-white/20'}`}>{i+1}</div><div className="flex -space-x-2"><Avatar name={t.p1?.name} colorIdx={t.p1?.avatar_idx} size="xs"/><Avatar name={t.p2?.name} colorIdx={t.p2?.avatar_idx} size="xs"/></div><div className="flex-1 min-w-0"><div className="font-bold text-white text-sm truncate">{t.name}</div><RecentForm results={t.recent} /></div><div className="text-right"><div className="text-emerald-400 font-black text-lg">{t.score}</div><div className="text-[10px] text-white/30">PTS</div></div></div>))}
                  {rankTab === 'rising' && stats.rising.map((p, i) => (<div key={p.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3" onClick={()=>setRadarModal({show:true, data:p})}><div className="text-emerald-400 font-bold text-lg w-6">+{p.score}</div><Avatar name={p.name} colorIdx={p.avatar_idx} size="sm"/><div className="flex-1"><div className="font-bold text-white">{p.name}</div><div className="text-xs text-white/40">近5场</div></div><TrendingUp className="text-red-400" size={18} /></div>))}
                </div>
              </div>
            )}
          ...
          {/* Bottom Nav */}
          {view !== 'match' && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-3 w-full justify-center px-4 pointer-events-none">
