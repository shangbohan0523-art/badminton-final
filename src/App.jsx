...
const Avatar = ({ name, colorIdx, size = 'md', className = '' }) => {
  const sizes = { xs: 'w-6 h-6 text-[8px]', sm: 'w-8 h-8 text-[10px]', md: 'w-12 h-12 text-sm', lg: 'w-16 h-16 text-xl', xl: 'w-20 h-20 text-2xl' };
  return (
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-4">
                 <div className="flex gap-2 bg-slate-900 p-1 rounded-xl border border-white/10">
                   <button onClick={()=>setLobbyTab('player')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${lobbyTab==='player'?'bg-emerald-500 text-white shadow':'text-slate-400 hover:text-white'}`}>选手</button>
                   <button onClick={()=>setLobbyTab('pair')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${lobbyTab==='pair'?'bg-emerald-500 text-white shadow':'text-slate-400 hover:text-white'}`}>组合</button>
                 </div>
              ...
              {lobbyTab === 'player' ? (
                  ...
                  {stats.player.map(p => {
                    ...
                    return (
                      <div key={p.id} className="relative group" onClick={() => !isManageMode && (isSel ? setSelectedPlayers(prev => prev.filter(id=>id!==p.id)) : selectedPlayers.length < 4 && setSelectedPlayers(prev=>[...prev, p.id]))}>
                        <div className={`w-full p-3 rounded-2xl border flex items-center gap-3 transition-all shadow-md cursor-pointer active:scale-95 ${isSel ? 'bg-emerald-500/10 border-emerald-400 ring-1 ring-emerald-400/50' : 'bg-slate-800/40 border-white/5'} ${isManageMode ? 'opacity-60' : ''}`}>
                          <Avatar name={p.name} colorIdx={p.avatar_idx} size="sm"/>
                          <div className="text-left flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5"><span className="font-bold text-white truncate text-base">{p.name}</span><span className="text-[10px] font-black bg-yellow-500/20 text-yellow-400 px-1 rounded">{p.rating}</span></div>
                            <div className="text-xs text-white/40 flex items-center gap-2"><span>{p.team_name}</span>{p.similarPro && <span className="text-emerald-400/70 bg-emerald-500/5 px-1 rounded flex items-center gap-0.5 z-20"><Star size={8}/> {p.similarPro.name}</span>}</div>
                            <RecentForm results={p.recent} />
                          </div>
                          <button onClick={(e)=>{e.stopPropagation(); setRadarModal({show:true, data:p})}} className="p-2 text-white/20 hover:text-white hover:bg-white/10 rounded-full z-20"><Info size={18}/></button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                   {stats.pair.length > 0 ? stats.pair.map(pair => (
                      <div key={pair.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3 relative">
                         ...
                         <button onClick={(e)=>{e.stopPropagation(); setRadarModal({show:true, data:pair, isPair:true})}} className="absolute top-2 right-2 p-1.5 text-white/10 hover:text-white"><Info size={14}/></button>
                      </div>
            ...
            <div className="flex flex-col gap-4">
               <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-2">
                    <select className="bg-slate-900 border border-white/10 rounded-lg text-xs p-1.5 outline-none text-white/80" value={currentSeasonId} onChange={(e)=>setCurrentSeasonId(e.target.value)}>
                       <option value="all">全部赛季</option>
                       {seasons.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    <button onClick={()=>setSeasonModal(true)} className="p-1.5 bg-white/10 rounded-lg"><Plus size={14}/></button>
                 </div>
               </div>
        ...
        {view === 'match' && prediction && (
           <div className="h-full flex flex-col max-w-md mx-auto pb-10">
             ...
             <div className="mb-4 px-2">
               ...
               <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden flex mb-2"><div style={{width: `${prediction.rateA}%`}} className="h-full bg-emerald-500 transition-all duration-1000"></div><div style={{width: `${prediction.b}%`}} className="h-full bg-blue-500 transition-all duration-1000"></div></div>
