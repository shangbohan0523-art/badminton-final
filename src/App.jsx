...
export default function App() {
  ...
  const [seasons, setSeasons] = useState([]); // 赛季列表
  ...
  const [currentSeasonId, setCurrentSeasonId] = useState('all'); // 'all' 或 season.id
  ...
  const [seasonModal, setSeasonModal] = useState(false); // 赛季管理弹窗
  ...
  const stats = useMemo(() => {
    ...
    if (currentSeasonId !== 'all') {
      ...
      if (season) {
        ...
        const end = new Date(season.end_date).getTime() + 86400000; // 包含结束当天
    ...
    const recentMatch = filteredMatches.slice(0, 5); // 也是基于赛季筛选后的
    ...
    return { 
      ...
      team: Object.values(tStats).sort((a, b) => b.score - a.score), // 新增队伍排行数据
  ...
  const handleUpdateScore = (team, delta) => {
    ...
    if (w) finishGame(w, ns); // 点击 + 号仍保留自动结束的快捷性
  };
            ...
            <div className="flex flex-col gap-4">
               <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-2">
                    <select className="bg-slate-900 border border-white/10 rounded-lg text-xs p-1.5 outline-none" value={currentSeasonId} onChange={(e)=>setCurrentSeasonId(e.target.value)}>
