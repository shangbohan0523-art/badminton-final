...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans relative overflow-hidden flex flex-col">
      ...
      {view !== 'match' && (
        <header className="relative z-20 px-4 pt-10 pb-3 flex justify-between items-center bg-slate-950/80 backdrop-blur border-b border-white/5">
      ...
      <main className={`relative z-10 w-full flex-1 overflow-y-auto px-3 pt-4 pb-32`}>
            ...
            {lobbyTab === 'player' ? (
                  ...
                  return (
                    <div key={p.id} className="relative group" onClick={() => !isManageMode && (isSel ? setSelectedPlayers(prev => prev.filter(id=>id!==p.id)) : selectedPlayers.length < 4 && setSelectedPlayers(prev=>[...prev, p.id]))}>
                      <div className={`w-full p-3 rounded-2xl border flex items-center gap-4 transition-all shadow-md active:scale-95 ${isSel ? 'bg-emerald-500/10 border-emerald-400 ring-1 ring-emerald-400/50' : 'bg-slate-800/40 border-white/5'} ${isManageMode ? 'opacity-60' : ''}`}>
                        ...
                        <button onClick={(e)=>{e.stopPropagation(); setRadarModal({show:true, data:p})}} className="p-2 text-white/20 hover:text-white hover:bg-white/10 rounded-full z-20"><Info size={20}/></button>
                      ...
                      {isManageMode && (<div className="absolute inset-0 flex items-center justify-center gap-3 bg-slate-900/80 backdrop-blur-sm rounded-2xl z-30"><button onClick={(e) => {e.stopPropagation(); setPlayerModal({ show: true, type: 'edit', data: p })}} className="p-2 bg-blue-500 rounded-full shadow-xl"><Edit2 size={16} /></button></div>)}
                    </div>
                  )
                })}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 pb-20">
                 {stats.pair.length > 0 ? stats.pair.map(pair => (
                    <div key={pair.id} className="bg-slate-800/40 border border-white/5 rounded-2xl p-3 flex items-center gap-3 relative" onClick={()=>setRadarModal({show:true, data:pair, isPair:true})}>
                       ...
                       <button onClick={(e)=>{e.stopPropagation(); setRadarModal({show:true, data:pair, isPair:true})}} className="absolute top-3 right-3 p-1 text-white/10 hover:text-white"><Info size={16}/></button>
                    </div>
      ...
      <Modal isOpen={radarModal.show && !radarModal.isPair}><div className="space-y-4"><h3 className="text-center font-bold text-white/50 uppercase tracking-wider text-xs">个人分析</h3>{radarModal.data ? (<div className="flex flex-col items-center"><RadarChart data={radarModal.data} label="" color="#10B981" /><div className="mt-4 text-center"><div className="text-xl font-black text-white">{radarModal.data.name}</div><div className="text-xs text-white/40 mb-4">综合评分: {radarModal.data.rating}</div>{radarModal.data.similarPro && (<div className="w-full bg-slate-800/50 border border-white/10 rounded-xl p-4 text-left relative overflow-hidden"><div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/10 blur-2xl rounded-full pointer-events-none" /><div className="flex items-center gap-2 mb-2"><div className="bg-gradient-to-br from-yellow-400 to-orange-500 w-8 h-8 rounded-full flex items-center justify-center text-slate-900 font-bold text-xs shadow-lg">{radarModal.data.similarPro.name[0]}</div><div><div className="text-sm font-bold text-white flex items-center gap-2">{radarModal.data.similarPro.name}<span className="text-[10px] font-normal bg-white/10 px-1.5 py-0.5 rounded text-white/60">相似度 90%+</span></div><div className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">{radarModal.data.similarPro.style}</div></div></div><div className="text-xs text-white/70 leading-relaxed border-t border-white/5 pt-2 mt-2"><Sparkles size={12} className="inline mr-1 text-yellow-400"/>{radarModal.data.similarPro.desc}</div></div>)}</div></div>) : null}<button onClick={()=>setRadarModal({show:false})} className="w-full py-3 bg-slate-800 rounded-xl text-white font-bold mt-4">关闭</button></div></Modal>
