...
// --- 静态配置 (确保包含 desc 描述) ---
const PRO_PLAYERS = [
  { name: '林丹', gender: 'M', style: '全能控制 / GOAT', desc: '羽毛球历史最伟大的运动员。早期以残暴的后场突击著称，后期转型为太极控制大师，拥有无与伦比的心理素质和大赛统治力，是完美的六边形战士。', attrs: { attr_power: 9, attr_speed: 9, attr_endurance: 9, attr_mentality: 10, attr_forehand: 9, attr_backhand: 8, attr_attack: 10, attr_defense: 9 } },
  { name: '李宗伟', gender: 'M', style: '极致速度 / 突击', desc: '无冕之王，拥有羽坛最顶级的步法和弹跳能力。他的变速突击和网前技术极具观赏性，擅长以快制快，长期占据世界排名第一。', attrs: { attr_power: 7, attr_speed: 10, attr_endurance: 9, attr_mentality: 7, attr_forehand: 9, attr_backhand: 8, attr_attack: 10, attr_defense: 8 } },
  { name: '陶菲克', gender: 'M', style: '反手天才 / 网前', desc: '印尼羽毛球天才，以极其细腻的网前手感和世界第一的反手杀球闻名。他的球风慵懒而华丽，是羽毛球技术流的巅峰代表。', attrs: { attr_power: 7, attr_speed: 7, attr_endurance: 6, attr_mentality: 8, attr_forehand: 8, attr_backhand: 10, attr_attack: 8, attr_defense: 7 } },
  { name: '安赛龙', gender: 'M', style: '巨人进攻 / 覆盖', desc: '丹麦巨人，利用身高臂长构建了超大的防守覆盖面积，配合极具威胁的高点下压进攻，开启了男子单打的新时代。', attrs: { attr_power: 10, attr_speed: 7, attr_endurance: 9, attr_mentality: 9, attr_forehand: 9, attr_backhand: 7, attr_attack: 10, attr_defense: 8 } },
  { name: '傅海峰', gender: 'M', style: '传奇重炮 / 后场', desc: '拥有时速400+公里杀球记录的传奇男双选手。他的后场进攻势大力沉，是“风云组合”中定海神针般的进攻核心。', attrs: { attr_power: 10, attr_speed: 8, attr_endurance: 8, attr_mentality: 9, attr_forehand: 9, attr_backhand: 6, attr_attack: 10, attr_defense: 7 } },
  { name: '戴资颖', gender: 'F', style: '假动作 / 魔术师', desc: '女单技术流巅峰，球风怪异灵动，擅长使用各种假动作欺骗对手重心，极具创造力，比赛观赏性极高。', attrs: { attr_power: 6, attr_speed: 8, attr_endurance: 8, attr_mentality: 7, attr_forehand: 10, attr_backhand: 9, attr_attack: 8, attr_defense: 6 } },
  { name: '安洗莹', gender: 'F', style: '天才防守 / 磨王', desc: '韩国天才少女，拥有令人绝望的防守能力和跑动能力，失误极少。擅长通过多拍相持消耗对手，被称为“女版桃田”。', attrs: { attr_power: 7, attr_speed: 8, attr_endurance: 10, attr_mentality: 9, attr_forehand: 8, attr_backhand: 8, attr_attack: 7, attr_defense: 10 } },
  { name: '陈雨菲', gender: 'F', style: '大帝稳健 / 拉吊', desc: '国羽女单领军人物，打法极其稳健，防守反击能力极强，心理素质过硬，是“大心脏”的代表。', attrs: { attr_power: 7, attr_speed: 8, attr_endurance: 9, attr_mentality: 10, attr_forehand: 8, attr_backhand: 8, attr_attack: 7, attr_defense: 9 } },
  { name: '马林', gender: 'F', style: '狮子吼 / 速度', desc: '西班牙名将，以极快的移动速度和凶狠的进攻著称，比赛中气势逼人，是女子打法男性化的代表。', attrs: { attr_power: 9, attr_speed: 10, attr_endurance: 8, attr_mentality: 9, attr_forehand: 9, attr_backhand: 7, attr_attack: 10, attr_defense: 7 } },
];
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans relative overflow-hidden flex flex-col">
      ...
      {view !== 'match' && (<header className="relative z-20 px-4 pt-10 pb-3 flex justify-between items-center bg-slate-950/80 backdrop-blur border-b border-white/5"><h1 onClick={()=>setView('lobby')} className="text-2xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-300 cursor-pointer">SMASH.</h1><button onClick={()=>setExpenseModal(true)} className="bg-white/5 p-2 rounded-full hover:bg-white/10"><Wallet size={20} className="text-emerald-400"/></button></header>)}
      
      <main className={`relative z-10 w-full flex-1 overflow-y-auto px-4 pt-4 pb-32 transition-all ${view === 'match' ? 'max-w-3xl mx-auto' : 'max-w-7xl mx-auto'}`}>
        {view === 'lobby' && (
          ...
          <div className="space-y-6 pb-24">
            ...
            <div className="flex flex-col gap-4">
               <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-2"><select className="bg-slate-900 border border-white/10 rounded-lg text-xs p-1.5 outline-none text-white/80" value={currentSeasonId} onChange={(e)=>setCurrentSeasonId(e.target.value)}><option value="all">全部赛季</option>{seasons.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><button onClick={()=>setSeasonModal(true)} className="p-1.5 bg-white/10 rounded-lg"><Plus size={14}/></button></div>
               </div>
